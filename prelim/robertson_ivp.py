...
def get_safe_random_y0():
    """Returns a y0 array with the same conditions as get_random_y0, but y1 is guaranteed to not be much larger than y2 and y3."""
    rng = np.random.default_rng()

    # Two random numbers uniformly chosen between 0.25 and 0.5 and the third is what's left to make sum(y0) == 1.0
    y01 = rng.uniform(0.25, 0.5)
    y02 = rng.uniform(0.25, 0.5)
    y03 = 1.0 - (y01 + y02)
...
def get_robertson_ivp(gen_func=get_random_y0):
    """Returns an instance of stiff_functions.IVPTuple for the Robertson Chemical Kinetics IVP with randomly chosen y0 provided by gen_func (defaults to get_random_y0)."""
