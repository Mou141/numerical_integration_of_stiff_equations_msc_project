...
def calc_err_stats(abs_err):
    ...
    return ErrorStatsTuple(
        l_2_norm(abs_err), l_infinity_norm(abs_err), np.mean(abs_err)
    )
...
def parse_cmd_args(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Performs statistical analysis on linear error data."
    )
    ...
    parser.add_argument(
        ...
        dest="graph_file",
        type=str,
        help="File to write histogram to.",
        default=None,
        required=False,
    )
    parser.add_argument(
        ...
        type=str,
        default="sqrt",
        help="Histogram binning method to use (see NumPy documentation).",
        required=False,
    )
...
def read_data_file(file_path):
    ...
    if (
        len(data.shape) == 1
    ):  # If the data contains only one dimension, then data is from a 1D IVP...
        return np.array([data])  # Wrap it in another array so that it is a 2D array

    else:  # If data is 2D...
        return (
            data.T
        )  # Transpose it so that columns and rows are switched (i.e. data[0] will return the errors in y1, data[1] the errors in y2 etc.)
...
def make_histogram(data, file_path=None, bins="sqrt"):
    ...
    figure, ax = plt.subplots(
        1, ndim, sharey=True
    )  # Create a list of subplots, one for each dimension of the error data. All the subplots are on the same row and share a y-axis

    if ndim == 1:  # If there is only one dimension...
        ax.set_xlabel("err(y)")  # Set the label for the x-axis
        ax = [
            ax
        ]  # Put the axis in a list since only one will be returned by the plt.subplots function
    ...
    ax[0].set_ylabel(
        "N"
    )  # Set the label for the y-axis (only one since the axis is shared)
...
def print_stats(data):
    ...
    if ndim == 1:  # If only one dimension...
        l2, l_inf, mean = calc_err_stats(
            data[0]
        )  # Get the statistics of that dimension
...
def main(data_file_path, graph_file_path=None, bins="sqrt"):
    ...
    make_histogram(
        data, file_path=graph_file_path, bins=bins
