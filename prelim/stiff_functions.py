...
STIFF_IVP = IVPTuple(
    ODEFunction=initial_value_problem,
    SolutionFunction=initial_value_solution,
    y0=np.array([1.0]),
    t0=0.0,
)
...
def initial_value_problem2(t, y):
    ...
    y1_prime = (
        (9.0 * y[0]) + (24.0 * y[1]) + (5.0 * np.cos(t)) - ((1.0 / 3.0) * np.sin(t))
    )
    y2_prime = (
        (-24.0 * y[0]) - (51.0 * y[1]) - (9.0 * np.cos(t)) + ((1.0 / 3.0) * np.sin(t))
    )
...
STIFF_IVP2 = IVPTuple(
    ODEFunction=initial_value_problem2,
    SolutionFunction=initial_value_solution2,
    y0=np.array([(4.0 / 3.0), (2.0 / 3.0)]),
    t0=0.0,
)
