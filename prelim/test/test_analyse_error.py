...
class TestStatsFunctions:
    ...
    l2_3 = (
        np.array([5.0, 1.5, 3.22, 5.6]),
        np.sqrt(np.mean(np.array([5.0, 1.5, 3.22, 5.6]) ** 2)),
    )
    l2_4 = (
        np.array([-2.0, 2.0]),
        2.0,
    )  # 2^2 = 4, (-2)^2 = 4, sum = 8, mean = 4.0, sqrt(4) = 2
    ...
    l_inf_1 = (
        np.array([2.0, 3.5, -1.0, 5.4]),
        5.4,
    )  # 5.4 is the largest value so it should be returned
    l_inf_2 = (
        np.array([-6.5, 4.3, -3.0, 2.22]),
        6.5,
    )  # 4.3 is the largest value, but -6.5 has the largest magnitude, so 6.5 should be returned
    ...
    stats_1 = (
        np.array([2.0, -2.0]),
        (2.0, 2.0, 0.0),
    )  # l2 is 2.0, l_inf is 2, mean is 0
    stats_2 = (
        np.array([3.5, 12.3, -33.4]),
        (np.sqrt(np.mean(np.array([3.5, 12.3, -33.4]) ** 2)), 33.4, -5.866666666666666),
    )
    ...
    def test_calc_err_stats(self, arr, stats):
        ...
        stats = analyse_error.ErrorStatsTuple._make(
            stats
        )  # Convert stats to an ErrorStatsTuple

        calc_stats = analyse_error.calc_err_stats(
            arr
        )  # Calculate the statistics on the array
...
class TestCmdArgs:
    ...
    args_1 = (
        ["test.tsv"],
        ("test.tsv", None, "sqrt"),
    )  # The graph file should be None if no path is specified for it
    args_2 = (
        ["test.tsv", "--graph-file", "test.png"],
        ("test.tsv", "test.png", "sqrt"),
    )  # Both data file and graph file path should be returned as input
    args_3 = (
        ["test.tsv", "--histogram-method", "fd"],
        ("test.tsv", None, "fd"),
    )  # Histogram method specified
    args_4 = (
        ["test.tsv", "-gf", "test.png"],
        ("test.tsv", "test.png", "sqrt"),
    )  # Both data file and graph file path should be returned as input
    args_5 = (
        ["test.tsv", "-hm", "fd"],
    ...
    fail_args_2 = [
        ...
        "test.png",
        "fgdgdg",
    ]  # Array with 3 arguments should fail as this is too many
    ...
    fail_args_6 = [
        "test.tsv",
        "--histogram-method",
    ]  # Switch given but no histogram binning method specified
    fail_args_7 = [
        "test.tsv",
        "-hm",
    ]  # Switch given but no histogram binning method specified
...
class TestDataFileRead:
    ...
    def data_file(self, tmp_path):
        ...
        basename = "tmp_{0}.tsv".format(
            random.randint(0, 1000000)
        )  # Generate a base filename of the format tmp_N.tsv, where N is a random integer between 0 and 100000

        full_path = (
            tmp_path / basename
        )  # Create a full file path from the path of the temporary directory and the basename
    ...
    arr_1 = np.array(
        [np.array([3.3, 545.43, 6766.76, 8.887])]
    )  # Test error data for 1D problem
    arr_2 = np.array(
        [[5.654, 56767.65, 8787.6, 64.23], [76.66666, 232.232, 1232.32, 8757566.00]]
    )  # Test error data for 1D problem
    arr_3 = np.array(
        [np.array([np.nan, 6.5454, 2323.2])]
    )  # Test that program processes NaN correctly
    ...
    def test_read_data_file(self, arr, data_file):
        ...
        np.savetxt(
            data_file, arr.T, delimiter="\t"
        )  # Write the array to file, transposing it

        data = analyse_error.read_data_file(data_file)  # Read the data back

        assert data == pytest.approx(
            arr, nan_ok=True
        )  # Check that the data is the same after reading
        assert len(data.shape) == 2  # Check that the array is 2D
        assert (
            data.shape == arr.shape
        )  # Check that the data and the original array are the same shape
...
class TestHistogram:
    ...
    def data_file(self, tmp_path):
        ...
        basename = "tmp_{0}.png".format(
            random.randint(0, 1000000)
        )  # Generate a base filename of the formar tmp_N.png, where N is a random integer between 0 and 1000000
    ...
    data_2 = np.array(
        [np.random.standard_normal(1000), np.random.standard_normal(1000)]
    )  # 2D data
