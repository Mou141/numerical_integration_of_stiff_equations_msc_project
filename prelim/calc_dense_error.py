...
def get_continous_solutions(t_cont, results):
    ...
    for method, solution in results.items():
        if solution.success:  # If integration was successful for this method...
            out[method] = solution.sol(
                t_cont
            )  # Calculate the continuous solution over the specified range

        else:
            print(
                "Integration failed for method '{0}' ('{1}').".format(
...
def get_errors(y_analytical, cont_solutions):
    ...
    for (
        ...
        cont_solutions.items()
    ):  # Iterate over method names and associated continuous, analytical solution...
        lin_err = calc_error.linear_error(
            y_num, y_analytical
        )  # Calculate linear error of continuous, analytical solution
        frac_err = calc_error.fractional_error(
            lin_err, y_num
        )  # Calculate fractional error of continuous, analytical solution
...
def save_analytical(t_cont, y_analytical):
    ...
    data = [
        t_cont,
        y_analytical.T,
    ]  # Data to save to each file (y is transposed into columns but t doesn't need to be as it's 1D)
...
def save_results(results):
    ...
    for (
        method,
        solution,
    ) in results.items():  # For each method and associated solution...
        if solution.success:  # If the integration completed successfully
            paths = [
                "t_numerical_{0}.tsv".format(method),
                "y_numerical_{0}.tsv".format(method),
            ]  # Paths to save the data to
            data = [
                solution.t,
                solution.y.T,
            ]  # Data to save to file (transposing y into columns)

            for fname, arr in zip(
                paths, data
            ):  # Iterate over paths and data in parallel...
                np.savetxt(
                    fname, arr, delimiter="\t"
                )  # Save each array to specified files
...
def save_continuous_and_error(cont_solutions, cont_errors):
    ...
    methods = list(
        cont_solutions.keys()
    )  # Get the names of the methods that were successful

    # Iterate over each method and its associated continuous numerical solution and linear and fractional errors
    for method, y_cont, errs in zip(
        methods, map(cont_solutions.get, methods), map(cont_errors.get, methods)
    ):
        paths = [
            "y_continuous_{0}.tsv".format(method),
            "linear_error_continuous_{0}.tsv".format(method),
            "fractional_error_continuous_{0}.tsv".format(method),
        ]  # Paths to save data to
        data = [
            y_cont.T,
            (errs[0]).T,
            (errs[1]).T,
        ]  # Data to save to file (transposing into columns)

        for fname, arr in zip(
            paths, data
        ):  # Iterate over file names and arrays in parallel...
            np.savetxt(fname, arr, delimiter="\t")  # Save each array to specified files
...
def save_data(t_cont, y_analytical, results, cont_solutions, cont_errors):
    ...
    save_continuous_and_error(
        cont_solutions, cont_errors
    )  # Save the (continous) numerical solutions to file and the associated linear and fractional errors
...
def plot_error(ndim, t_cont, cont_errors):
    ...
    figure, ax = plt.subplots(
        ndim, 1, sharex=True
    )  # Create 1 subplot for each dimension of the IVP
    ...
    methods = list(
        cont_errors.keys()
    )  # Get a list containing the names of all the methods

    figure.legend(
        methods, loc="upper right"
    )  # Add a legend to the graph with the names of all the methods in order

    for lin_err, _ in map(
        cont_errors.get, methods
    ):  # For each method's tuple of error values...
...
def test_integrators_continuous(
    end_t, ivp, N, integrators=test_ivp.INTEGRATORS, atol=1.0e-06, rtol=1.0e-03
):
    ...
    ivp = stiff_functions.IVPTuple._make(
        ivp
    )  # Convert ivp to an IVP Tuple if it isn't one

    t_cont = np.linspace(
        ivp.t0, end_t, N
    )  # Create an array of t points of length N, linearly spaced between t0 and end_t
    ...
    results = test_ivp.test_integrators(
        end_t, ivp, integrators=integrators, dense_output=True, atol=atol, rtol=rtol
    )
    cont_solutions = get_continous_solutions(t_cont, results)
    cont_errors = get_errors(y_analytical, cont_solutions)

    save_data(
        t_cont, y_analytical, results, cont_solutions, cont_errors
    )  # Save data to file
    plot_error(
        ivp.ndim, t_cont, cont_errors
    )  # Plot the linear error on a graph and display it
